...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("BTCGold version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  BTCGoldd [options]                     " + "\n" +
                  "  BTCGoldd [options] <command> [params]  " + _("Send command to -server or BTCGoldd") + "\n" +
                  "  BTCGoldd [options] help                " + _("List commands") + "\n" +
                  "  BTCGoldd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "BTCGold:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: BTCGold.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: BTCGoldd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 33813 or testnet: 32819)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 33812 or testnet: 32810)") + "\n" +
        ...
        "\n" + _("SSL options: (see the BTCGold Wiki for SSL setup instructions)") + "\n" +
...
//    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        if (nTransactionFee > 0.001 * 500 * COIN)
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BTCGold is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("BTCGold version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "BTCGold server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BTCGold") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart BTCGold to complete") << "\n";
