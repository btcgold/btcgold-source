...
// Copyright (c) 2011-2012 BTCGold Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BTCGoldTEST0\r");
            Send(hSocket, "WHO #BTCGoldTEST0\r");
        } else {
            // randomly join #BTCGold00-#BTCGold99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #BTCGold%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BTCGold%02d\r", channel_number).c_str());
        }
