...
uint256 hashGenesisBlock("0xa2808bcd9649cac3e51045b324a581d2c7ddc353a12e78e151bba41934f743d2");
...
const string strMessageMagic = "BTCGold Signed Message:\n";
...
{
    // BTCGold: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BTCGold: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "btcgold genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x97af18fe2bd03c6cfe31b77ed11f3b578f85b12d542c5ba1a38916c96ce6060c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd8, 0x9a, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BTCGoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BTCGoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BTCGoldMiner(CWallet *pwallet)
{
    printf("BTCGoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BTCGold-miner");
    ...
    try { loop {
        ...
        printf("Running BTCGoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BTCGoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BTCGoldMiner, pwallet));
}
